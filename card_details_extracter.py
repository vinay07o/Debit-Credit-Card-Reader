...
def find_details(loc):
    ...
    print("Car Type:", FIRST_NUMBER[card_number[0]])
    ...
    resultant_dict = {"Card Holder Name": card_name, "Card Number" :card_number,
                     "Card Expair On": card_expiry, "Card Type": FIRST_NUMBER[card_number[0]]}
    ...
    if len(after_expiry)>1:
        ...
        if possible_names:
            for i in possible_names:
                ...
                try:
                    card_name = p.match(i).groupdict()
                except:
                    pass

        if card_name:
            return card_name['First']
    return None